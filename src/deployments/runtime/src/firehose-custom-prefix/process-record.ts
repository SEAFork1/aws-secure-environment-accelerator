...
function processPrefix (data, records, mappings) {
  
    console.log(`Record LogGroup: ${data.logGroup}`);

    let serviceName = null;

    if (mappings) {
      for (const mapping of mappings) {
        if (data.logGroup.indexOf(mapping.logGroupPattern) >= 0) {
  ...
  const approxArrivalTimestamp = new Date(records[0].approximateArrivalTimestamp);
...
function processRecords (records, mappings) {
  return records.map(r => {
    ...
    if (data.messageType === 'CONTROL_MESSAGE') {
    ...
    } else if (data.messageType === 'DATA_MESSAGE' && 'logGroup' in data) {
      ...
      const partitionKeys = {dynamicPrefix: processPrefix(data, records, mappings)};
      return {
        data: encodedData,
        result: 'Ok',
        recordId: recId,
...
module.exports.handler = async function (event, context) {
  ...
  const records = processRecords(event.records, mappings)
  let projectedSize = 0
